import { buildTransport, emailShell, tr, CONTACT_EMAIL } from '@/lib/mailer';
...
async function sendCancelEmail(data) {
  ...
  if (!transport || !data.email) return;
    ...
    html: emailShell({
      ...
      body: `
        <p style="color:#333;margin:0 0 20px;font-size:15px;line-height:1.7">
          ...
          ${L
            ? 'Vi bekræfter hermed, at din booking er blevet annulleret.'
...
export async function PATCH(request) {
  ...
  try {
    ...
    await sendCancelEmail(data).catch(() => {});

...
export function botAvatarSvg(size) {
  // Unique per call: this helper is rendered more than once per page, and the
  // four gradient ids below were hardcoded, producing duplicate ids in the
  // document and two SVGs referencing the same <defs>.
  const u = `bav${++bavSeq}`;
  ...
  <defs>
    ...
    <linearGradient id="${u}-bav-gold" x1="14%" y1="6%" x2="86%" y2="96%">
    ...
    <linearGradient id="${u}-bav-glass" x1="0%" y1="0%" x2="0%" y2="100%">
      <stop offset="0%"   stop-color="#ffffff" stop-opacity=".20"/>
      <stop offset="55%"  stop-color="#ffffff" stop-opacity="0"/>
    </linearGradient>
    <!-- refined emerald lens -->
    <radialGradient id="${u}-bav-eye" cx="34%" cy="26%" r="82%">
    ...
    <linearGradient id="${u}-bav-smile" x1="0%" y1="0%" x2="100%" y2="0%">
      <stop offset="0%"   stop-color="#9a7712"/>
      <stop offset="50%"  stop-color="#f3d873"/>
      <stop offset="100%" stop-color="#9a7712"/>
    </linearGradient>
    <filter id="${u}-bav-glow" x="-90%" y="-90%" width="280%" height="280%">
      <feGaussianBlur stdDeviation="2.6" result="b"/>
      <feMerge><feMergeNode in="b"/><feMergeNode in="SourceGraphic"/></feMerge>
    </filter>
    <filter id="${u}-bav-soft" x="-120%" y="-120%" width="340%" height="340%">
      <feGaussianBlur stdDeviation="4.5"/>
    </filter>
    <filter id="${u}-bav-drop" x="-25%" y="-25%" width="150%" height="155%">
  ...
  <ellipse cx="50" cy="90" rx="34" ry="7" fill="#d4af37" opacity=".18" filter="url(#${u}-bav-soft)"/>

  <!-- antenna -->
  <rect x="48.4" y="7" width="3.2" height="16" rx="1.6" fill="url(#${u}-bav-gold)"/>
  <circle cx="50" cy="6" r="6"   fill="#37e08a" opacity=".5" filter="url(#${u}-bav-soft)"/>
  <circle cx="50" cy="6" r="3.4" fill="url(#${u}-bav-gold)"/>
  <circle cx="48.8" cy="4.9" r="1.1" fill="#fff8d6"/>

  <!-- ===== HEAD ===== -->
  <!-- outer gold frame -->
  <rect x="14" y="20" width="72" height="72" rx="19" fill="url(#${u}-bav-gold)" filter="url(#${u}-bav-drop)"/>
  ...
  <rect x="18.5" y="24.5" width="63" height="63" rx="15" fill="url(#${u}-bav-gold-in)"/>
  <!-- glass face -->
  <rect x="21" y="27" width="58" height="58" rx="12.5" fill="url(#${u}-bav-face)"/>
  <!-- curved glass-dome reflection -->
  <path d="M21 39.5 Q50 22 79 39.5 L79 33 Q79 27 73 27 L27 27 Q21 27 21 33 Z" fill="url(#${u}-bav-glass)"/>

  <!-- ===== EYES (refined lenses) ===== -->
  <!-- left -->
  <circle cx="38" cy="52" r="10.5" fill="#040c08"/>
  <circle cx="38" cy="52" r="9.4"  fill="#00d36a" opacity=".30" filter="url(#${u}-bav-glow)"/>
  <circle cx="38" cy="52" r="7.4"  fill="url(#${u}-bav-eye)"/>
  ...
  <circle cx="62" cy="52" r="9.4"  fill="#00d36a" opacity=".30" filter="url(#${u}-bav-glow)"/>
  <circle cx="62" cy="52" r="7.4"  fill="url(#${u}-bav-eye)"/>
  ...
  <!-- ===== SMILE ===== -->
  <path d="M40 68 Q50 75.5 60 68" stroke="url(#${u}-bav-smile)" stroke-width="2.4" stroke-linecap="round" fill="none"/>

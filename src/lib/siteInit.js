...
// KV-driven content is injected via innerHTML (legacy renderer), so it must be
// sanitized. Regex stripping is bypassable (nested tags, <svg/onload>, split
// attributes), so parse into an inert document and keep only an allow-list of
// formatting tags/attributes — the browser's own parser decides what a tag is.
var SAFE_TAGS={STRONG:1,B:1,EM:1,I:1,U:1,BR:1,P:1,SPAN:1,UL:1,OL:1,LI:1,SMALL:1,A:1};
var KNOWN_EXT_ALL={};PKGS.forEach(function(p){(p.relevant||[]).concat(p.includes||[]).forEach(function(id){KNOWN_EXT_ALL[id]=1;});});
// Resolve an extra to its canonical built-in id. Admin-saved extras used to
// get a random UUID, which broke package "included" matching (the Guld package
// then re-sold motorrens/lak as paid add-ons). Fall back to matching on name.
var CANON_EXT_NAMES={};
[["motor","motorrens","engine clean"],["lak","lak & glansbeskyttelse","lak- & glansbeskyttelse","paint & gloss protection"],
 ["pleje","indvendig pleje & beskyttelse","interior care & protection"],["haar","fjernelse af dyrehår","pet hair removal"],
 ["saede","sæderens (stof)","seat clean (fabric)"],["barnesaede","barnesæde rens","rens af barnesæde","child seat cleaning"]]
 .forEach(function(row){for(var i=1;i<row.length;i++)CANON_EXT_NAMES[row[i]]=row[0];});
function extId(ex){
  if(!ex)return '';
  if(CANON_EXT_NAMES[ex.id]||/^(motor|lak|pleje|haar|saede|barnesaede)$/.test(ex.id))return ex.id;
  var n=ex.name;
  var cands=[];
  if(typeof n==='string')cands.push(n);
  else if(n&&typeof n==='object'){if(n.da)cands.push(n.da);if(n.en)cands.push(n.en);}
  for(var i=0;i<cands.length;i++){
    var k=String(cands[i]).trim().toLowerCase();
    if(CANON_EXT_NAMES[k])return CANON_EXT_NAMES[k];
  }
  return ex.id;
}
function sanitizeHtml(v){
  if(typeof v!=='string')return v;
  if(v.indexOf('<')<0)return v; // plain text — nothing to do
  try{
    var doc=new DOMParser().parseFromString('<div id="r">'+v+'</div>','text/html');
    var root=doc.getElementById('r');
    (function walk(node){
      var kids=Array.prototype.slice.call(node.children);
      kids.forEach(function(el){
        if(!SAFE_TAGS[el.tagName]){
          // Drop the element but keep its text so copy isn't lost.
        ...
        Array.prototype.slice.call(el.attributes).forEach(function(a){
          ...
          var ok=(el.tagName==='A'&&(n==='href'||n==='target'||n==='rel'));
          // Reject any javascript:/data: URL (whitespace + entities stripped first)
          if(ok&&n==='href'){
            var val=(a.value||'').replace(/[\s -]/g,'').toLowerCase();
            if(/^(javascript|data|vbscript):/.test(val))ok=false;
          }
    ...
    // No DOMParser (shouldn't happen in a browser) — fall back to plain text.
    return v.replace(/[<>]/g,'');
  }
}
function sanitizeDeep(o){
  if(typeof o==='string')return sanitizeHtml(o);
  if(Array.isArray(o))return o.map(sanitizeDeep);
  if(o&&typeof o==='object'){var r={};for(var k in o)r[k]=sanitizeDeep(o[k]);return r;}
  return o;
}
if(overrides.prices){CARS.forEach(function(c){if(overrides.prices[c.id])Object.assign(c.prices,overrides.prices[c.id]);});}
if(overrides.extras&&overrides.extras.length)EXTRAS=sanitizeDeep(overrides.extras);
...
// Days-open summary for the contact card, reflecting closedDays + language.
function hoursDaysLabel(langOverride){
  var da=(langOverride||LANG)!=='en';var closed=HOURS.closedDays||[];
  if(!closed.length)return da?'Alle dage':'Every day';
  if(closed.length>=7)return da?'Lukket':'Closed';
  var DA=['søndag','mandag','tirsdag','onsdag','torsdag','fredag','lørdag'];
  var EN=['sunday','monday','tuesday','wednesday','thursday','friday','saturday'];
  var names=closed.map(function(d){return da?DA[d]:EN[d];});
  var last=names.length>1?names.slice(0,-1).join(', ')+(da?' og ':' and ')+names[names.length-1]:names[0];
  return (da?'Alle dage undtagen ':'Every day except ')+last;
}
// Reflect the configured hours on the contact card so the displayed opening
// time + days always match what the booking grid offers (run on each lang switch).
function applyHoursDisplay(){
  try{
    var ht=document.querySelector('.cl-item-hours .htime');if(ht)ht.textContent=HOURS.open+' – '+HOURS.close;
    var hd=document.querySelector('.cl-item-hours .hday');if(hd){hd.textContent=hoursDaysLabel();hd.removeAttribute('data-i18n');}
  }catch(e){}
}
var REVIEWS=[];
var FAQ=[
...
function renderCars(){
  ...
  CARS.forEach(function(c){
    ...
    d.innerHTML='<span class="car-check"><svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="3.5" stroke-linecap="round" stroke-linejoin="round"><polyline points="20 6 9 17 4 12"/></svg></span><div class="car-stage"><img class="car-photo" src="/cars/'+c.id+'.webp" alt="'+c.label[LANG]+'" loading="lazy" decoding="async" onerror="this.remove()" onload="if(this.nextElementSibling)this.nextElementSibling.style.display=\'none\'"><svg class="cs" viewBox="0 0 120 64" fill="none" stroke="currentColor" stroke-width="2" stroke-linejoin="round" stroke-linecap="round"><g transform="translate(120,0) scale(-1,1)">'+c.svg+'</g></svg></div><div class="nm">'+c.label[LANG]+'</div><div class="ex">'+c.ex[LANG]+'</div><div class="car-price"><span class="car-price-from">'+(LANG==='da'?'Fra':'From')+'</span> '+fmtKr(minPrice)+'</div>';
...
function renderCalc(){
  ...
  document.getElementById('calcIcon').innerHTML='<img class="calc-photo" src="/cars/'+selCar.id+'.webp" alt="'+selCar.label[LANG]+'" decoding="async" onerror="this.remove()" onload="if(this.nextElementSibling)this.nextElementSibling.style.display=\'none\'">'+svgWrap(selCar.svg,60,34,'#9af0bd');
...
function applyLang(){
  ...
  document.querySelectorAll('[data-i18n-ph]').forEach(function(e){var k=e.getAttribute('data-i18n-ph');if(I18N[LANG][k]!==undefined)e.setAttribute('placeholder',I18N[LANG][k]);});
  document.querySelectorAll('[data-i18n-aria]').forEach(function(e){var k=e.getAttribute('data-i18n-aria');if(I18N[LANG][k]!==undefined)e.setAttribute('aria-label',I18N[LANG][k]);});
...
/* Plate-lookup feature removed — its UI no longer exists in the DOM.
   Restore from git history if the plate input ever returns. */
...
function openWiz(car,pkg){
  ...
  try{var _wc=document.getElementById('wizClose');if(_wc)_wc.focus();}catch(e){}
}
...
// Close the booking wizard on Escape (matches the × and backdrop).
document.addEventListener('keydown',function(e){if(e.key==='Escape'&&modal.classList.contains('open'))closeWiz();},{signal:__sig});
...
function drawWiz(){
  ...
  if(step===1){
    ...
    CARS.forEach(function(c){var minP=Math.min(c.prices.udv,c.prices.indv,c.prices.hele);h+='<div class="opt'+(wiz.car&&wiz.car.id===c.id?' sel':'')+'" data-car="'+c.id+'"><div class="opt-icon-wrap"><img class="opt-photo" src="/cars/'+c.id+'.webp" alt="" loading="lazy" decoding="async" onerror="this.remove()" onload="if(this.nextElementSibling)this.nextElementSibling.style.visibility=\'hidden\'">'+svgWrap(c.svg,46,26)+'</div><span class="opt-lbl">'+c.label[LANG]+'</span><span class="opt-ex">'+c.ex[LANG]+'</span><span class="opt-fra">'+(LANG==='da'?'Fra ':'From ')+fmtKr(minP)+'</span><span class="opt-time">'+c.time[LANG]+'</span></div>';});
    h+='</div>';
  }else if(step===2){
    ...
    var optionalExtras=EXTRAS.filter(function(ex){
      var cid=extId(ex);
      if(pkgIncludes.indexOf(cid)>=0)return false;         // already paid for in this package
      return pkgRelevant.indexOf(cid)>=0||!KNOWN_EXT[cid];
    });
    if(pkgIncludes.length>0){
      ...
      EXTRAS.filter(function(ex){return pkgIncludes.indexOf(extId(ex))>=0;}).forEach(function(ex){
    ...
    // Server rejects bookings more than 28 days ahead — mirror that in the picker
    var maxD=(function(){var d=new Date();d.setDate(d.getDate()+28);return new Intl.DateTimeFormat('sv-SE',{timeZone:'Europe/Copenhagen'}).format(d);})();
  ...
  if(step===5){
    ...
    if(fd){
      fd.addEventListener('change',function(){
        wiz.date=fd.value;wiz.time='';
        if(!fd.value){ // cleared the date: stop polling, show a neutral hint
          if(_slotPollTimer){clearInterval(_slotPollTimer);_slotPollTimer=null;}
          var g=document.getElementById('slotGrid');
          if(g)g.innerHTML='<div class="slot-hint" style="grid-column:1/-1">'+(LANG==='da'?'Vælg en dato for at se ledige tider.':'Pick a date to see available times.')+'</div>';
          return;
        }
        loadSlots(fd.value);
      });
  ...
  wizBody.querySelectorAll('[data-pkg]').forEach(function(o){o.addEventListener('click',function(){wiz.pkg=PKGS.filter(function(p){return p.id===o.dataset.pkg;})[0];wizBody.querySelectorAll('[data-pkg]').forEach(function(x){x.classList.remove('sel');});o.classList.add('sel');if(wiz.pkg){var rel=wiz.pkg.relevant||[];var inc=wiz.pkg.includes||[];wiz.extras=wiz.extras.filter(function(id){var ex=EXTRAS.filter(function(e){return e.id===id;})[0];var cid=ex?extId(ex):id;return inc.indexOf(cid)<0&&(rel.indexOf(cid)>=0||!KNOWN_EXT_ALL[cid]);});}});});
...
function slotMinutes(t){return parseInt(t.slice(0,2),10)*60+parseInt(t.slice(3,5),10);}
function isPastSlot(date,time){
  // The server rejects any slot at/before the current Copenhagen time,
  // so hide a today slot once we've reached it (minute-accurate for :30 slots).
  ...
  var nowMin=parseInt(now.slice(11,13),10)*60+parseInt(now.slice(14,16),10);
...
function submitBooking(cb){
  ...
  fetch('/api/book',{
    ...
    body:JSON.stringify({
      ...
      pkgId:wiz.pkg?wiz.pkg.id:null,
      extras:extList,
      extraIds:wiz.extras.slice(),
...
(function(){
  ...
  var FAQ={
    da:[
      ...
      {keys:['pris','kost','hvad kost','priser','billig','dyr'],
       ans:['Priserne starter fra <strong>500 kr</strong> for udvendig vask og <strong>800 kr</strong> for fuld pakke. Prisen afhænger af bilstørrelse og dit postnummer. <a href="#vaelg">👉 Se alle pakker her</a>',
            'Vi har pakker fra <strong>500 kr</strong> – ingen skjulte gebyrer! Prisen beregnes efter din bil og adresse. <a href="#vaelg">Sammenlign pakker →</a>'],
      ...
      {keys:['åbningstid','åbent','hvornår','åbnings','weekend','hverdage'],
       ans:['Vi holder åbent <strong>'+hoursDaysLabel('da').toLowerCase()+' kl. '+HOURS.open+'–'+HOURS.close+'</strong> – eller efter aftale. Online booking er tilgængelig hele døgnet 🕗'],
    ...
    en:[
      ...
      {keys:['price','cost','how much','prices','cheap','expensive'],
       ans:['Prices start from <strong>500 kr</strong> for exterior wash and <strong>800 kr</strong> for a full package. Price depends on car size and zip code. <a href="#vaelg">👉 See all packages</a>',
            'We offer packages from <strong>500 kr</strong> – no hidden fees! Price is calculated based on your car and address. <a href="#vaelg">Compare packages →</a>'],
      ...
      {keys:['zealand','area','cover','where','zip','distance'],
       ans:['We cover <strong>all of Zealand</strong> – Greater Copenhagen, North and South Zealand. Travel is <strong>free</strong> across our service area. <a href="#vaelg">Check price for your area →</a>'],
      ...
      {keys:['hours','open','when','weekend','weekday'],
       ans:['We are open <strong>'+hoursDaysLabel('en').toLowerCase()+' '+HOURS.open+'–'+HOURS.close+'</strong> – or by arrangement. Online booking is available 24/7 🕗'],
  ...
  toggle.addEventListener('click',function(){
    if(!opened){
      ...
      if(badge)badge.style.display='none';opened=true;document.body.classList.add('chat-open');
...
applyLang();
// Re-render the price calc on resize so the mobile package-tabs (which hide
// all but one card ≤880px) don't stay collapsed after resizing back to desktop.
var _pkgResizeT;
window.addEventListener('resize',function(){clearTimeout(_pkgResizeT);_pkgResizeT=setTimeout(function(){if(selCar)renderCalc();},160);},{signal:__sig});
}

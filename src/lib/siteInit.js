...
/* ====== KØRSEL (drive fee from zip) ====== */
function driveFee(zip){
  if(!zip||zip.length<3) return 0;
  var z=parseInt(zip,10);
  if(isNaN(z)) return 0;
  if(z>=1000&&z<=2999) return 149;      // København
  if(z>=3000&&z<=3699) return 199;       // Nordsjælland
  if(z>=4000&&z<=4099) return 99;        // Roskilde
  if(z>=4600&&z<=4699) return 99;        // Køge
  if(z>=4100&&z<=4399) return 0;         // Ringsted/Sorø nær base
  if(z>=4700&&z<=4999) return 0;         // Næstved nær base
  if(z>=4200&&z<=4299) return 79;        // Slagelse
  return 199;                            // øvrige
}

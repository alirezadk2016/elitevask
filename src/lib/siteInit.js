...
// KV-driven content is injected via innerHTML (legacy renderer), so strip
// script vectors from it: <script> blocks, inline on*= handlers and
// javascript: URLs. Formatting tags (<strong>, <br>, links) survive.
...
if(overrides.faq&&overrides.faq.length>=5)FAQ=sanitizeDeep(overrides.faq);
var I18N={
...
function renderCalc(){
  ...
  document.getElementById('calcIcon').innerHTML='<img class="calc-photo" src="/cars/'+selCar.id+'.png" alt="'+selCar.label[LANG]+'" decoding="async" onerror="this.remove()" onload="if(this.nextElementSibling)this.nextElementSibling.style.display=\'none\'">'+svgWrap(selCar.svg,60,34,'#9af0bd');
